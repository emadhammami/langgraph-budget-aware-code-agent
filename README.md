# LangGraph Budget-Aware Code Agent

[![CI](https://github.com/emadhammami/langgraph-budget-aware-code-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/emadhammami/langgraph-budget-aware-code-agent/actions/workflows/ci.yml)
[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1vje68dj4UjgrkMaFiWfvKV6NhzLAULX2?usp=sharing)
[![Python 3.11+](https://img.shields.io/badge/python-3.11%2B-blue)](https://www.python.org/)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)

A research prototype demonstrating how to build a **budget-aware, observable agentic
workflow** using [LangGraph](https://github.com/langchain-ai/langgraph) and a
Planner–Executor–Critic architecture, extended with **executable validation** for
evidence-based fix assessment.

> **Scope note:** This is an academic prototype, not a production system.  The primary
> contribution is the software-engineering design — explicit guardrails, typed shared state,
> deterministic routing, executable validation, and structured telemetry — rather than
> state-of-the-art code-repair performance.

---

## Why executable validation matters

LLM-generated solutions should be validated through executable evidence whenever possible.

LLMs reason over text.  They can propose syntactically plausible fixes that still fail at
runtime — a zero-division guard that uses the wrong variable name, an off-by-one error that
only manifests on certain inputs, or a fix that imports a non-existent module.  Static code
inspection (including LLM review) cannot reliably detect these failures.

Executable validation closes this gap by treating the **observed behaviour of running code**
as primary evidence:

| Signal | Source | Cost | Reliability |
|---|---|---|---|
| Syntax validity | AST parse | free | high (but narrow) |
| LLM review | language model | tokens | variable |
| **Execution outcome** | **subprocess run** | **CPU time** | **high (objective)** |

When a candidate fix is actually executed and fails with a `ZeroDivisionError` or times out,
that is an **objective, reproducible fact** — not an opinion.  The Critic can reason over
concrete evidence rather than hypothetical failure modes.

This is a step toward **trustworthy AI workflows**: agentic systems whose decisions are
grounded in observable reality rather than pure LLM reasoning.

---

## Why this project matters

Modern agentic systems face several unsolved engineering challenges.  This
prototype deliberately addresses five of them in a self-contained, inspectable
codebase:

| Challenge | How it is addressed here |
|---|---|
| **Planner–Executor–Critic architecture** | Three distinct LLM roles with explicit interfaces, preventing any single agent from both proposing and approving its own output. |
| **Executable validation** | A sandboxed subprocess executor runs candidate fixes and captures stdout, stderr, runtime, and exit status as objective evidence fed to the Critic. |
| **Guardrails and budget-aware orchestration** | A centralised `guardrail_router` enforces hard limits on loop count and cumulative token spend before every node transition. |
| **Observability / telemetry** | Every node calls `finalize_update`, which appends a structured snapshot (including validation metrics) to `metrics_history` — providing a full audit trail without additional infrastructure. |
| **Dependable agentic systems** | Explicit shared state (Pydantic models), deterministic routing, and early-exit ensure the graph always terminates cleanly. |

---

## Architecture

```
START
  │
load_file ──(guardrail_router)──► planner ──(guardrail_router)──► executor
                                      ▲                                │
                                      │                (guardrail_router)
                                      │                                ▼
                                      │                           validation   ← NEW
                                      │                (runs fix in subprocess)
                                      │                                │
                                      │                (guardrail_router)
                                      │                                ▼
                                      └──────────────────────────── critic
                                                 (if regions remain)

At any edge:  guardrail_router may redirect to → early_exit → finish → END
```

### Nodes

| Node | Role |
|---|---|
| `load_file` | Reads the target Python file, splits it into lines, and uses the standard `ast` module to discover function boundaries as *candidate regions*. |
| `planner` | Selects the single most suspicious unanalysed region and forms a bug hypothesis.  Increments `loop_count` each call. |
| `executor` | Proposes a minimal, targeted fix for the selected region. |
| `validation` | Executes the proposed fix in a sandboxed subprocess via `SandboxExecutor`. Captures stdout, stderr, return code, exit status, and runtime. Attaches results to state as `ValidationResult`. |
| `critic` | Validates the fix using three signals: (1) execution evidence from `validation_result` (primary), (2) a local AST syntax check, and (3) an LLM review (secondary). Grounds its decision in observed execution facts. |
| `early_exit` | Records a budget-exceeded message when either hard limit is breached. |
| `finish` | Terminal node; seals the final message for output. |

### Shared state (`AgentState`)

All inter-node communication passes through a single Pydantic model.  There are
no side channels.  Key fields:

- `candidate_regions` / `analyzed_regions` / `accepted_fixes` — analysis bookkeeping.
- `planner_output`, `executor_output`, `validation_result`, `critic_output` — typed outputs from each role.
- `validation_failures` — cumulative count of failed sandbox executions (for benchmarking).
- `loop_count`, `total_tokens`, `max_loops`, `max_tokens_budget` — guardrail counters.
- `metrics_history` — append-only telemetry log (one entry per node execution).
- `next_step` — routing intent set by each node; may be overridden by the router.

### ValidationResult fields

```python
class ValidationResult(BaseModel):
    success: bool           # True iff exit code == 0
    stdout: str             # captured standard output
    stderr: str             # captured standard error
    return_code: int        # subprocess return code
    runtime_seconds: float  # wall-clock execution time
    timed_out: bool         # True if process was killed by timeout
    error_category: str     # "none" | "timeout" | "syntax_error" | "memory_error" | "runtime_error"
```

### Guardrail router

```python
def guardrail_router(state: AgentState) -> str:
    if state.loop_count > state.max_loops or state.total_tokens > state.max_tokens_budget:
        return "early_exit"
    return state.next_step
```

This one function is attached to **every** conditional edge, so budget checks
happen between each pair of nodes — not only after the critic.

### SandboxExecutor

`agent/tools/sandbox_executor.py` runs candidate code in a child process with:

- **Hard timeout** — process is killed after `timeout` seconds (default 10 s).
- **Return code capture** — the subprocess exit code is stored with stdout, stderr, and runtime.
- **Soft memory limit** — applied via `RLIMIT_AS` on POSIX; silently skipped on Windows.
- **No shell** — code is written to a temp file; `subprocess.run` is called with a list of
  args (no `shell=True`), eliminating shell-injection risk.
- **Truncated output** — stdout and stderr are capped at 32 KiB each to prevent state bloat.

### Telemetry

Each call to `finalize_update` appends a snapshot to `metrics_history`:

```json
{
  "step": 3,
  "node": "validation",
  "loop_count": 2,
  "total_tokens": 1847,
  "accepted_fixes": 1,
  "analyzed_regions": 2,
  "budget_used_ratio": 0.23,
  "validation_success": false,
  "validation_return_code": 1,
  "validation_runtime_seconds": 0.08,
  "validation_error_category": "runtime_error",
  "validation_failures_total": 1
}
```

`visualize.py` plots accepted fixes and budget consumption over steps.

---

## Repository structure

```
langgraph-budget-aware-code-agent/
├── .github/
│   └── workflows/
│       └── ci.yml                    # GitHub Actions: pytest + ruff on py3.11/3.12
├── agent/
│   ├── __init__.py                   # Public API: exposes run_agent
│   ├── state.py                      # Pydantic state models (AgentState, ValidationResult, …)
│   ├── helpers.py                    # Shared utilities: token estimation, metrics, LLM client
│   ├── nodes.py                      # LangGraph node functions (incl. validation_node)
│   ├── router.py                     # Guardrail router
│   ├── graph.py                      # Graph assembly and run_agent entry point
│   └── tools/
│       ├── __init__.py
│       └── sandbox_executor.py       # NEW — subprocess-based sandboxed code runner
├── samples/
│   └── sample_buggy_file.py         # Minimal demo input with three intentional bugs
├── tests/
│   ├── conftest.py                   # Shared pytest fixtures
│   ├── test_state.py                 # AgentState and sub-model tests
│   ├── test_router.py                # guardrail_router branch coverage
│   ├── test_helpers.py               # Deterministic helper unit tests
│   ├── test_graph_smoke.py           # Graph wiring + mocked LLM end-to-end tests
│   ├── test_sandbox_executor.py      # NEW — SandboxExecutor unit tests
│   └── test_validation_node.py       # NEW — validation_node unit tests (mocked sandbox)
├── main.py                           # CLI entry point
├── visualize.py                      # Telemetry charts (matplotlib)
├── pyproject.toml                    # Project metadata, dependencies, ruff/pytest config
├── requirements.txt
└── README.md
```

---

## How to run

### Option A — Google Colab (no setup required)

Open the notebook directly in your browser and run all cells:

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1vje68dj4UjgrkMaFiWfvKV6NhzLAULX2?usp=sharing)

The notebook installs its own dependencies and prompts you for your Gemini API
key via `getpass`, so no local configuration is needed.

### Option B — Local Python environment

#### 1. Install dependencies

```bash
# Standard install
pip install -r requirements.txt

# Or, with dev tools (pytest + ruff)
pip install -e ".[dev]"
```

#### 2. Set your Gemini API key

```bash
# Linux / macOS
export GOOGLE_API_KEY=<your-key>

# PowerShell
$env:GOOGLE_API_KEY = '<your-key>'
```

#### 3. Run on the sample file

```bash
python main.py
```

#### 4. Run on a custom file

```bash
python main.py path/to/your_file.py
```

#### 5. Run the test suite (no API key required)

```bash
pytest
```

The agent will print a structured log to stdout as it runs, then output a final
report listing accepted fixes.  Two telemetry plots are displayed at the end.

### Expected output (sample)

```
Analysing: samples/sample_buggy_file.py
──────────────────────────────────────────────────
... (structured JSON log per node) ...

══════════════════════════════════════════════════
FINAL REPORT
══════════════════════════════════════════════════
Analysis complete.

Region: divide:lines 1-2
Summary: Add a zero-division guard
Fix:
def divide(a, b):
    if b == 0:
        raise ValueError("b must not be zero")
    return a / b

---

Region: greet:lines 4-6
Summary: Replace mutable default argument
Fix:
def greet(name=None):
    if name is None:
        name = []
    name.append("x")
    return "Hello " + str(name)
...
```

---

## Sample input

`samples/sample_buggy_file.py` contains three intentional Python bugs that
demonstrate concrete, recognisable failure modes:

| Function | Bug | Failure |
|---|---|---|
| `divide(a, b)` | No zero-division guard | `ZeroDivisionError` at runtime |
| `greet(name=[])` | Mutable default argument | State persists silently across calls |
| `get_item(items, idx)` | No bounds check | `IndexError` for out-of-range index |

---

## Limitations

This is a **prototype built for research demonstration purposes**.  Specific limitations:

- **Sandbox is subprocess-only.** The sandbox uses `subprocess.run` without Docker or a
  kernel-level seccomp policy.  Code with sufficient permissions could write files or open
  network connections.  For production use, a container-based sandbox is recommended.
- **Memory limits are POSIX-only.** `RLIMIT_AS` is unavailable on Windows; only timeout
  enforcement applies there.
- **Single-pass per region.** The executor proposes one fix; there is no retry loop within a
  region.
- **No file patching.** Accepted fixes are printed to stdout; the original file is not
  modified.
- **No benchmark harness yet.** The sandbox executes the fixed snippet in isolation without
  the project test suite or golden cases, so behavioural correctness depends on the snippet
  being self-contained.
- **Token estimation fallback.** When usage metadata is unavailable, token counts are
  approximated with a 4-chars-per-token heuristic.
- **Flat region granularity.** Regions are individual top-level functions; nested functions
  and class methods are not separately tracked.
- **Gemini-specific.** The LLM client is `langchain-google-genai`; swapping providers
  requires changing `build_llm` in `helpers.py`.

---

## Future work

The following extensions are planned for later research phases:

- [ ] **Benchmark harness** — run fixed candidates against known inputs, expected outputs,
  and eventually the project test suite; use exit codes and assertions as validation signals.
- [ ] **Retry loop in executor** — re-prompt if the critic rejects a fix, with a back-off
  counter to avoid infinite loops.
- [ ] **In-place file patching** — apply accepted fixes and write the corrected file.
- [ ] **Container-based sandbox** — replace the subprocess sandbox with a Docker/Podman
  container for stronger isolation.
- [ ] **Richer static analysis** — integrate `pylint` or `pyflakes` output as an additional
  Critic signal.
- [ ] **Configurable LLM provider** — make `build_llm` accept a provider argument.
- [ ] **CLI budget flags** — expose `--max-loops` and `--max-tokens` as `argparse` arguments.
- [ ] **Golden-file benchmarks** — validate fix quality against known-correct outputs.

---

## Research positioning

This prototype addresses a concrete open challenge in agentic AI systems: how do you build
a multi-step LLM workflow that **terminates reliably, stays within resource budgets, remains
fully auditable, and validates its outputs against objective evidence** — without sacrificing
modularity?

The addition of executable validation moves the Critic from *pure reasoning* to
*evidence-based reasoning*.  This is a meaningful step toward dependable agentic systems,
where workflow decisions are grounded in observable facts rather than LLM opinion alone.

The design priorities — reproducibility, observability, evaluation, and future
experimentation — are deliberately chosen over production features.  Every architectural
decision (typed state, append-only telemetry, deterministic routing, sandboxed execution)
serves the goal of making the system's behaviour inspectable and benchmarkable.

These properties are relevant to ongoing research in:

- trustworthy AI workflows
- testing of non-deterministic systems
- validation and guardrails for agentic systems
- dependable LLM-enabled program repair

---

## Environment

Tested with:

- Python 3.11 / 3.12
- `langgraph` 0.2.x
- `langchain-google-genai` 2.x (Gemini 2.5 Flash)
- `pydantic` 2.x

---

## License

MIT
